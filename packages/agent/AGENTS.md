...
- Activated definitions are unsupported in voice/realtime, including foreground envelope calls: realtime mappers throw `VoiceToolBridgeError`, voice decisions deny before approval matching (even replayed approvals), and the private fiber-local `VoiceToolDispatch` marker fences direct low-level bridge dispatch in the activated registry before validation/admission/inline effects. Do not expose the marker as public API or infer activation from business params. Resolve ordinary voice toolsets without a host.
...
- Loop emits `ToolExecutionAccepted` for results carrying `acceptance`; never `ToolExecutionCompleted`/`SubagentCompleted` and never usage. Client `AgentToolRun`, chat `ChatToolState`, and `ToolRunState` have a distinct `Accepted` tag that is inactive, non-error, and not completed.
- Approval ids for activated calls are `approval:<callId>:background-v1:<canonical JSON of name/execution/arguments>`; legacy ids stay `approval:<callId>`. `prepareToolBatch` rejects malformed envelopes with a synthetic `isError` result before prompting or launching.
