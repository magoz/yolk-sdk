...
- Vercel AI Gateway uses the OpenAI-compatible JSON Chat Completions endpoint at `https://ai-gateway.vercel.sh/v1/chat/completions` and lowers the host limit to Gateway `max_tokens`. Model ids remain opaque `provider/model` strings; optional fallback models and provider routing are host policy. Authentication accepts an AI Gateway API key or Vercel OIDC token as Bearer auth, required auth/content-negotiation headers override extras, and custom endpoint URLs are trusted credential-bearing proxies.
