...
- Host tool batches run with bounded concurrency from `LoopConfig.toolConcurrency` (default 4); preserve model-visible tool result order by original tool-call order.
