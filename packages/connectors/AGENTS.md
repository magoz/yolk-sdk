...
- Generic attachment retrieval uses the incoming connection and credential through optional `EmailClient.getAttachment`, preserving existing host adapters. Hosts resolve metadata IDs, return decoded file bytes as `contentBase64`, and own MIME parsing, size policy, storage, and scanning; POP3 may require fetching the whole UIDL-addressed message.
...
- Outlook attachment listing is metadata-only and includes inline attachments. File attachment retrieval maps Graph `contentBytes` to `contentBase64`; item/reference attachments remain list-only metadata, and hosts own decoding, size policy, storage, and scanning.
