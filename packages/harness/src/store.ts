...
export type SnapshotRunStoreOptions = {
...
export class RunStore extends Context.Service<RunStore, RunStoreApi>()(
  ...
  /**
   * Canonical owning layer for snapshot-backed claims. Each call builds a fresh layer so
   ...
   * composed harnesses never share `Ref` state across factory calls.
   */
  static inMemoryLayer = (): Layer.Layer<RunStore> =>
    Layer.effect(
      this,
      Effect.gen(function* () {
        ...
        return RunStore.of({
          ...
          resumeCount: runId =>
            Ref.get(resumes).pipe(Effect.map(current => current.get(runId) ?? 0))
        })
      })
    )
}

/** Backward-compatible delegation to {@link RunStore.snapshotLayer}. */
export const makeSnapshotRunStoreLayer = (
  options: SnapshotRunStoreOptions
): Layer.Layer<RunStore> => RunStore.snapshotLayer(options)

/** Backward-compatible delegation to {@link RunStore.inMemoryLayer}. */
export const makeInMemoryRunStoreLayer = (): Layer.Layer<RunStore> => RunStore.inMemoryLayer()
