...
describe('knowledge searching', () => {
  ...
  it.effect('ingests extracted documents through package services', () => {
    ...
    const store = {
      upsertDocument: (input: { readonly document: IndexedKnowledgeDocument }) => Effect.succeed(input.document),
      ...
      markDocumentReady: (input: { readonly title?: string; readonly summary?: string; readonly chunkCount: number }) => Effect.succeed({ ...document, title: input.title, summary: input.summary, chunkCount: input.chunkCount }),
  ...
  it.effect('searches vector matches with adjacent context', () => {
    const store = {
      ...
      markDocumentError: () => Effect.void,
      deleteDocument: () => Effect.void,
      searchChunks: () => Effect.succeed([{ chunk: { id: 'chunk_2', scopeId: 'scope_1', documentId: 'doc_1', content: 'match', position: 1, tokenCount: 1 }, score: 0.9, document }]),
      searchChunksByText: () => Effect.succeed([]),
      getContextChunks: () => Effect.succeed([{ id: 'chunk_1', scopeId: 'scope_1', documentId: 'doc_1', content: 'before', position: 0, tokenCount: 1 }, { id: 'chunk_2', scopeId: 'scope_1', documentId: 'doc_1', content: 'match', position: 1, tokenCount: 1 }])
    } satisfies SearchIndexStoreApi
    const layer = Layer.mergeAll(
      Layer.succeed(SearchIndexStore, store),
      Layer.succeed(KnowledgeEmbedder, { embedTexts: texts => Effect.succeed(texts.map(() => [1, 0])), embedQuery: () => Effect.succeed([1, 0]) })
    ...
    return Effect.gen(function* () {
      const results = yield* searchKnowledge({ scope: { _tag: 'KnowledgeScope', id: 'scope_1' }, query: 'alpha', mode: 'vector', contextChunks: 1 })
  ...
  it.effect('rejects invalid search inputs before services are required', () =>
    Effect.gen(function* () {
      ...
      const error = yield* searchKnowledge({ scope: { _tag: 'KnowledgeScopes', ids: [] }, query: 'alpha' }).pipe(
        Effect.flip,
  ...
  it.effect('adapts lookup as an agent tool', () => {
    const tool = makeKnowledgeLookupTool<{ readonly userId: string }>({
      ...
      get: () => Effect.fail(new ToolError({ tool: 'knowledge_lookup', message: 'unused', cause: 'execution' }))
    })

    return Effect.gen(function* () {
      const result = yield* tool.execute({
        context: { userId: 'user_1' },
        call: ToolCall.make({ id: 'call_1', name: 'knowledge_lookup', params: { operation: 'search', query: 'docs' } })
      })
