...
import { KnowledgeArtifactError } from '@yolk/knowledge/errors'
import type { PutKnowledgeArtifactInput } from '@yolk/knowledge/artifacts'
...
describeWithDb('createFileKnowledgeObject', () => {
  it.effect('stores original artifact and extracted representation', () => {
    ...
    return Effect.gen(function* () {
      ...
      const provenance = yield* db
        ...
        .where(eq(schema.knowledgeProvenance.objectId, object.id))
      ...
      yield* db
        ...
        .where(eq(schema.knowledgeObject.id, object.id))
  ...
  it.effect('deletes owned artifacts before deleting object rows', () => {
    ...
    const storageKey = `knowledge/${objectId}/original/${artifactId}`
    ...
    return Effect.gen(function* () {
      ...
      yield* db.insert(schema.knowledgeObject).values({
        id: objectId,
        userId,
        role: 'source',
        title: 'Delete me',
      ...
      yield* deleteKnowledgeObject({ userId, id: objectId })

      const objects = yield* db
        .select()
        .from(schema.knowledgeObject)
        .where(eq(schema.knowledgeObject.id, objectId))
      const artifacts = yield* db
        .select()
        .from(schema.knowledgeArtifact)
