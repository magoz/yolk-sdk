...
- `no-node-deps-in-agent-tools` applies only to non-test files under `lib/agents/tools/*`; it bans Node imports, `@effect/platform-node`, `@yolk-sdk/mcp/client/node`, and raw `fetch()`.
