...
**Example:** MCP client tests use fake `HttpClient` layers for remote JSON/SSE and the reusable `@yolk/mcp-server` fixture in `packages/mcp-server/test/fixtures/fake-stdio-mcp-server.ts` for local stdio.
