...
This document defines usability standards for the application. All UI work must follow these guidelines.
...
### 2.3 Remember Values Between Attempts
...
| -------------- | ---------------- | ------------------------------------------ |
| Main list      | No items yet     | Create your first item to get started      |
| Search results | No results found | Try adjusting your search terms or filters |
| Filtered list  | No matches       | No items match the current filters         |
| Activity feed  | No activity yet  | Actions will appear here as they happen    |
...
| Permission     | "Access denied"          | "You don't have permission to edit this resource"             |
...
<ErrorState
  ...
  action={<Button onClick={() => router.push('/')}>Go to Dashboard</Button>}
/>
```
...
<Alert variant="success">Organization created successfully</Alert>
```
