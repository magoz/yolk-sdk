...
- Root `pnpm test:run` runs the app's `test:db:push` before Vitest.
- DB-dependent tests skip when required env such as `DATABASE_URL` is absent.
- Package tests prefer explicit fakes/fixtures; add and document any real-service dependency before using it.

For a real service layer, keep acquisition scoped and share it with `layer()` only when tests are independent under shared state.
