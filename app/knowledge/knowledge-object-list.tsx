...
import { DeleteKnowledgeObjectButton } from './delete-knowledge-object-button'
...
const previewText = (item: KnowledgeObjectListItem) => {
...
const artifactDownloadHref = (input: { readonly objectId: string; readonly artifactId: string }) =>
  `/api/knowledge/artifacts?objectId=${encodeURIComponent(input.objectId)}&artifactId=${encodeURIComponent(input.artifactId)}`

export function KnowledgeObjectList({ items }: { readonly items: ReadonlyArray<KnowledgeObjectListItem> }) {
          ...
          <div className="p-4">
            <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_120px_170px_auto] md:items-center">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <DeleteKnowledgeObjectButton id={item.object.id} label={item.object.title} onDeleteOptimistic={removeOptimisticItem} />
              </div>
            </div>
          </div>
              ...
              <div className="space-y-1">
                {item.artifact ? (
                  <>
                    ...
                    <p>
                      <a className="font-medium text-foreground underline underline-offset-4 hover:text-muted-foreground" href={artifactDownloadHref({ objectId: item.object.id, artifactId: item.artifact.id })}>
                        Download artifact
                      </a>
                    </p>
                  </>
