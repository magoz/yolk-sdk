...
export const getKnowledgeDocuments = (input: { readonly userId: string; readonly collectionId: string }) =>
  Effect.gen(function* () {
    const db = yield* Db
    const rows = yield* db
    ...
    Effect.mapError(error => new AppSearchIndexStoreError({ message: 'Could not get knowledge search documents', cause: error }))
  )
