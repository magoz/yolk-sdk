...
describe('knowledgeArtifactStorageKey', () => {
  it('builds stable original artifact keys', () => {
    expect(
      knowledgeArtifactStorageKey({ objectId: 'obj_1', artifactId: 'art_1', kind: 'original' })
    ).toBe('knowledge/obj_1/original/art_1')
  })

  it('builds stable derived artifact keys', () => {
    expect(
      knowledgeArtifactStorageKey({ objectId: 'obj_1', artifactId: 'art_2', kind: 'extracted_text' })
    ).toBe('knowledge/obj_1/derived/text/art_2.txt')
    expect(
      knowledgeArtifactStorageKey({ objectId: 'obj_1', artifactId: 'art_3', kind: 'thumbnail', extension: '.webp' })
