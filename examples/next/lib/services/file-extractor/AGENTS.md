...
- App-local service only; do not move provider/file parsing dependencies into `@yolk-sdk/knowledge`.
- Keep DB writes and knowledge search ingestion in domain helpers (`examples/next/lib/core/storage/*`, `examples/next/lib/core/knowledge/*`); this service only extracts text.
