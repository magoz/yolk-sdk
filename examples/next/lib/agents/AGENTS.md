...
Configured in `examples/next/lib/agents/text-agent-config.ts`; selected by UI and routed in `examples/next/app/api/agent/route.ts` / `cloudflare/agent/src/yolk-agent.ts`:
...
- App tools in `examples/next/lib/agents/tools/*` must be runtime-portable: no Node-only imports, no raw `fetch()`, use Effect `Config`/`HttpClient`/Schema and injected adapters.
...
- `tools/storage-search-tool.ts`: storage source discovery + storage search tools; Next/Workflow text and voice over app knowledge search/DB adapters; omitted from Cloudflare bootstrap
...
- Requires per-user Codex OAuth token from `examples/next/lib/core/agent/openai-codex-auth.ts`
...
- Device-flow server actions live in `examples/next/lib/core/agent/*-action.ts`; they redirect unauthenticated users, save/delete tokens, and revalidate `/agent`
...
- Requires per-user Claude OAuth token from `examples/next/lib/core/agent/anthropic-claude-auth.ts`
