...
const CreateFileStorageUploadUrlActionLayer = Layer.mergeAll(AppLayer, R2KnowledgeUploadStoreLayer)

const safeFilename = (filename: string) => filename.trim().replaceAll('/', '_').replaceAll('\\', '_')

export const createFileStorageUploadUrlAction = async (input: {
  ...
  return await NextEffect.runPromise(
    Effect.gen(function* () {
      ...
      const storageKey = `uploads/storage/${session.user.id}/${createId()}/${filename}`
      ...
      Effect.withSpan('action.storage.createFileUploadUrl'),
      Effect.provide(CreateFileStorageUploadUrlActionLayer),
      Effect.scoped,
      ...
      Effect.tapError(error => reportError(error, { operation: 'action.storage.createFileUploadUrl' })),
