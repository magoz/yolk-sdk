...
describeWithDb('createFileKnowledgeDocument', () => {
  it.effect(
    'stores original file and chunks extracted content',
    () => {
      ...
      return Effect.gen(function* () {
        ...
        Effect.provide(
          Layer.succeed(KnowledgeFileBlobStore, {
            ...
            getFile: input =>
              Effect.fail(
                new KnowledgeFileError({ message: `Unexpected getFile ${input.storageKey}` })
              ),
            deleteFile: input =>
              Effect.fail(
                new KnowledgeFileError({ message: `Unexpected deleteFile ${input.storageKey}` })
              )
          })
        ),
        Effect.provide(
  ...
  it.effect(
    'keeps upload bytes when extraction detaches its input buffer',
    () => {
      const userId = createId()
      const puts: Array<PutKnowledgeFileInput> = []
            ...
            extract: input =>
              Effect.sync(() => {
                ...
                throw new FileExtractionError({
                  message: 'Expected ArrayBuffer input',
                  format: 'pdf'
                })
              })
          })
  ...
  it.effect(
    'deletes owned files before deleting document rows',
    () => {
      const userId = createId()
      ...
      return Effect.gen(function* () {
        ...
        Effect.provide(
          Layer.succeed(KnowledgeFileBlobStore, {
            putFile: input =>
              Effect.fail(
                new KnowledgeFileError({ message: `Unexpected putFile ${input.storageKey}` })
              ),
            getFile: input =>
              Effect.fail(
                new KnowledgeFileError({ message: `Unexpected getFile ${input.storageKey}` })
