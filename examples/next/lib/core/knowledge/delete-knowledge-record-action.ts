...
export const deleteKnowledgeRecordAction = async (id: string) => {
  await cookies()

  return await NextEffect.runPromise(
    Effect.gen(function* () {
      const session = yield* getSession()
      yield* deleteKnowledgeRecord({ userId: session.user.id, id })
    }).pipe(
      ...
      Effect.catch(() => Effect.succeed({ _tag: 'Error' as const, message: 'Could not delete knowledge record' }))
    )
  )
}
