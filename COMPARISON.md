...
const AppLayer = Layer.mergeAll(
  Bus.defaultLayer, Config.defaultLayer, Session.defaultLayer, /* ...40 more... */
...
const PermissionLayer = Layer.effect(ToolExecutor, Effect.gen(function* () {
  const inner = yield* ToolExecutor
  return ToolExecutor.of({
    execute: (call) => pipe(checkPermission(call), Effect.flatMap(() => inner.execute(call))),
  })
}))
```
